...
if(isset($_POST['dataToExcel']))
{
	...
	$objPHPExcel = PHPExcel_IOFactory::load("../../modules/template/template_month_plan.xls");
  ...
  foreach(unserialize($_POST['dataToExcel']) as $key=>$row)
  {
		...
		$aSheet->setCellValue('D'.$cell,$row['Name_Project']);
		$aSheet->setCellValue('E'.$cell,$row['Name_Customer']);
...
header('Content-Disposition: attachment;filename="saveExcelMonthPlan.xls"');
