...
print "<a href=\"../Graph/graphReport.php?Type=hourReport\">График загрузки подразделений по месяцам</a><br />\n";
print "<CENTER><H2>Объектная карточка 'Трудоемкость'</H2></CENTER><br />\n";
...
for ($i = 1; $i < 13; $i++)
{
	$Query1 = "SELECT department.Name_Department, SUM(design.Man_Hour_Sum) AS Man_Hour_Sum ".
	...
	if ( !($dbResult = mysql_query($Query, $link)) )
	{
		...
		{
			//округлим до 2 знака после запятой
