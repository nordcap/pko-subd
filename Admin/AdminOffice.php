...
+-----------------------------------------------------------------------+
Данный модуль является страницей администрирования бюро
+-----------------------------------------------------------------------+
...
if ($_SESSION['Name_Post'] == "Администратор")
{
	...
	print "<H2>Модуль редактирования бюро</H2><br />\n";
	...
	if ( isset($_REQUEST['hidden']) )
	{
		...
		switch ( $_REQUEST['rbselect'] )
		{
			...
			if ( !($dbResult = mysql_query($Query, $link)) )
			{
				...
				{
					$_REQUEST['Name_Office'] = $row['Name_Office'];
				}
				$tmpNameOffice = $_REQUEST['Name_Office'];
			}
			break;
		}
		if( count($errors) > 0 )
	...
	print "<Tr>\n";
	print "<Td>Наименование бюро \n";
	print "<Td><Input Type=\"text\" Name=\"Name_Office\" Value=\"$tmpNameOffice\" class=\"size2\">\n";
	print "<Tr>\n";
