...
+-----------------------------------------------------------------------+
Данный модуль является страницей администрирования марок работ
+-----------------------------------------------------------------------+
...
if ($_SESSION['Name_Post'] == "Администратор")
{
	...
	print "<H2>Модуль редактирования марок</H2><br />\n";
	...
	if ( isset($_REQUEST['hidden']) )
	{
		...
		switch ($_REQUEST['rbselect'])
		{
			...
			if ( !($dbResult = mysql_query($Query, $link)) )
			{
				...
				{
					$_REQUEST['Name_Mark'] = $row['Name_Mark'];
					$_REQUEST['Comment_Mark'] = $row['Comment_Mark'];
				}
				$tmpNameMark = $_REQUEST['Name_Mark'];
				$tmpCommentMark = $_REQUEST['Comment_Mark'];
			}
			break;
		}
		if( count($errors) > 0 )
	...
	print "<Tr>\n";
	print "<Td>Марка \n";
	print "<Td><Input Type=\"text\" Name=\"Name_Mark\" Value=\"$tmpNameMark\" class=\"size2\">\n";
	print "<Tr>\n";
	print "<Td>Название марки\n";
	print "<Td><Input Type=\"text\" Name=\"Comment_Mark\" Value=\"$tmpCommentMark\" class=\"size2\">\n";
	print "<Tr>\n";
